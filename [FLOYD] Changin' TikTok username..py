...
def main():
    ...
    session_id = input("введите session id: ")

    last_username = get_profile(session_id, device_id, iid)
    if last_username != "None": #ء 
        print(f"теперь ваш юзернейм: {last_username}")
        new_username = input("введите желаемый юзернейм: ")
        print(change_username(session_id, device_id, iid,last_username, new_username))

    else:
        print("неправильный session ID или другая ошибка.")
    print("telegram @FloydRocky")
    # telegram @FloydRocky
